...
func (dbOp *CRUD) query(c *gin.Context) {
	// connect to DB
...
func (dbOp *CRUD) CreateTable(db *sql.DB) error {
	...
	if _, err := db.Exec(dbOp.sql); err != nil {
	...
	dbOp.sql = `INSERT IGNORE INTO fruits (fid,fName,fPrice,fNum,fType)
